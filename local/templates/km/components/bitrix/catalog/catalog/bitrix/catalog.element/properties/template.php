<?if(!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED!==true)die();?>
	<div class="nav_goods_block">
	<?if($arResult["PAGER"]["PREV"][0] || $arResult["PAGER"]["NEXT"][1]):
		$prev = $arResult["PAGER"]["PREV"][0] ? $arResult["PAGER"]["PREV"][0] : $arResult["PAGER"]["NEXT"][1];?>
		<a class="nav_goods_item nav_goods_item_prev" href="<?=$prev["DETAIL_PAGE_URL"]?>" title="<?=$prev["NAME"]?>">
			<span class="nav_goods_icon nav_goods_icon_prev"></span>
			<div class="wrap_goods_img">
				<img src="<?=ResizeImage($prev["DETAIL_PICTURE"], 112, 112, true)?>" alt="<?=$prev["NAME"]?>"/>
			</div>
		</a>	
	<?endif?>
	<?if($arResult["PAGER"]["NEXT"][0] || $arResult["PAGER"]["PREV"][1]):
		$next = $arResult["PAGER"]["NEXT"][0] ? $arResult["PAGER"]["NEXT"][0] : $arResult["PAGER"]["PREV"][1];?>
		<a class="nav_goods_item nav_goods_item_next" href="<?=$next["DETAIL_PAGE_URL"]?>" title="<?=$next["NAME"]?>">
			<span class="nav_goods_icon nav_goods_icon_next"></span>
			<div class="wrap_goods_img">
				<img src="<?=ResizeImage($next["DETAIL_PICTURE"], 112, 112, true)?>" alt="<?=$next["NAME"]?>"/>
			</div>
		</a>	
	<?endif?>
		<div class="clearfix"></div>
		<a href="<?=$arResult["SECTION"]["SECTION_PAGE_URL"]?>" class="return_to_list_icon">к списку товаров</a>					
	</div>
	<div class="about_current_goods">
	<?if(!empty($arResult["DISPLAY_PROPERTIES"])):?>
		<ul class="characteristics_list">
		<?foreach($arResult["DISPLAY_PROPERTIES"] as $arProperty)
			if(!empty($arProperty["DISPLAY_VALUE"])):?>
			<li><?=$arProperty["NAME"]?>: <?=$arProperty["DISPLAY_VALUE"]?></li>
			<?endif?>
		</ul>
	<?endif?>
		<div class="price_current_goods">
		<?if($arResult['MIN_PRICE']['PRINT_DISCOUNT_VALUE'] != $arResult['MIN_PRICE']['PRINT_VALUE']):?>
			<div class="old_price_current_goods">
				<span class="price_type">Старая цена</span>
				<span class="line_through_span">
					<span class="old_price_goods"><?=$arResult['MIN_PRICE']['PRINT_VALUE']?></span>
				</span>
			</div>
			<div class="new_price_current_goods">
				<span class="price_type">Новая цена</span>
				<span class="number_price"><?=$arResult['MIN_PRICE']['PRINT_DISCOUNT_VALUE']?></span>
			</div>
		<?else:?>
			<div class="new_price_current_goods">
				<span class="price_type">Цена</span>
				<span class="number_price"><?=$arResult['MIN_PRICE']['PRINT_DISCOUNT_VALUE']?></span>
			</div>
		<?endif?>
		</div>
	</div>
	<a href="<?=$arResult["QUICK_BUY"]?>" class="btn order_call_btn iframe_order_call js_modal">заказать звонок</a>
	<div class="adding_info_current_goods">
	<?foreach($arResult["SHOPS"] as $arShop):?>
		<div class="adding_info_item">
			<span class="adding_info_item_lbl">
				<img src="<?=SITE_TEMPLATE_PATH?>/static/images/available_icon.png">
				<span><?if($arShop["ISSET"] == "Y"):?>В наличии<?else:?>Под заказ<?endif?></span>
			</span>
			<span class="adding_info_phone_number">т.: 8 <?=$arShop["PROPERTIES"]["CODE"]["VALUE"]?> <?=$arShop["PROPERTIES"]["PHONE"]["VALUE"]?></span>
			<span class="adding_info_adress"><?=$arShop["PROPERTIES"]["CITY"]["VALUE"]?>, <?=$arShop["PROPERTIES"]["ADDRESS"]["VALUE"]?> </span>
			<a href="#" class="on_map_link">на карте</a>						
		</div>
	<?endforeach?>
		<?/*<div class="adding_info_item">
			<span class="adding_info_item_lbl">
			...
			<span class="adding_info_phone_number">т.: 8 800 775 0161</span>
			<span class="adding_info_adress">Ростов-на-Дону, Грибоедовский, 6 </span>
			<a href="#" class="on_map_link">на карте</a>						
		</div>
		<div class="adding_info_item">
			<span class="adding_info_item_lbl">
				<img src="<?=SITE_TEMPLATE_PATH?>/static/images/order_icon.png">
				<span>Под заказ</span>
			</span>
			<span class="adding_info_phone_number">т.: 8 800 775 01 7</span>
			<span class="adding_info_adress">Краснодар, ул.Тургенева, 76</span>
			<a href="#" class="on_map_link">на карте</a>	
			<p>Указанная цена ориентировочная и зависит от комплектации</p>
		</div>*/?>
		<div class="adding_info_item">
			<p>Указанная цена ориентировочная и зависит от комплектации</p>
		</div>
		<div class="adding_info_item">
			<span class="adding_info_item_lbl">
				<img src="<?=SITE_TEMPLATE_PATH?>/static/images/delivery_icon.png">
				<span>Доставка и оплата</span>
			</span>
			<p>Доставка – Ростов (бесплатно) <br> Сборка в день доставки</p>						
		</div>					
